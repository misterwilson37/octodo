...
| **Repo vs. latest drop** | **app 1.30.1 · store 0.21.2 · css 0.54.0 · html 0.48.1.** ⚠️ **FOUR drops went out in one session:** 1.29.0/0.21.0/0.47.0 and 1.29.1/0.21.1/0.47.1 both preceded this one and are SUPERSEDED, not equivalent. Check the badge against this row before believing any bug report from that day. **Verify with the version badge, don't assume:** hover the version in the header, compare against the banner at the top of this file. Anything lower means the web files need re-uploading. `store.js`, `queue.js`, `celebrate.js`, `config.js` were NOT touched this session. |
...
Group **BASE** — the app on a multi-tenant floor.

1. ✅ **BASE-1. It builds you a workspace.** Sign in at the dev URL → a working app with **three** tiers (Home, Work, Personal), not Jake's six.
2. ✅ **BASE-2. It builds you exactly ONE.** Reload, sign out and in. `workspaces/` holds one document, not one per visit.
3. ✅ **BASE-3. The bones still work.** Add a task, check it off → confetti. Add a project → it appears.
4. ✅ **BASE-4. The pipeline template is EMPTY (E16).** ⚙️ ▸ Pipeline: no stages. *Jake reported "Template is empty" — that is the PASS condition, not a fault.* Katie's thirteen actuarial steps must travel with **her** workspace at migration and must never be born into a new one.
5. ✅ **BASE-5. The badge tells the truth.** Hover the version number: the module list, then a line reading `workspace <id>`, then the doc census. **Compare it against the banner at the top of THIS file — do not compare it against numbers written in this test.** *(The old wording hard-coded `app.js 1.22.0 …`, so it went stale every single drop and Jake correctly ignored it. A test that has to be edited on every release will not be.)* If css reports anything but the banner's number, stop — that is the 1.x doubling bug in a new house.
6. ✅ **BASE-6. THE IMPORTANT ONE — isolation.** A second Google account gets its **own** workspace with its own three tiers and **cannot see Jake's tasks**. Console: two `workspaces/` documents, different `ownerEmail`s. *This is E1, and it is the entire security model.*
7. ⚠️ **BASE-7. `completedBy` is landing.** **PARTIAL — tasks pass, project STAGES were being erased.** See §5b-i. Re-run after app 1.30.1: complete a stage, **then edit the project's stage list**, then confirm the finished stage still carries `completedBy`. **Where to look, because "the console" was not enough detail:** [console.firebase.google.com](https://console.firebase.google.com) ▸ your project ▸ **Build ▸ Firestore Database** ▸ **Data** tab ▸ `workspaces` ▸ click your workspace id ▸ `tasks` ▸ click any task you have completed. The right-hand pane lists its fields; you want **`completedBy`**, holding your email in lowercase. If it is absent or `null` on a completed task, stop — that is what keeps Reflection from crediting you with someone else's work now that tiers are shared (§7.2).
8. ✅ **BASE-8. Sign-out is clean.** ⏻ returns you to the sign-in screen, not a blank page.
9. ✅ **BASE-9. You can sign in as somebody ELSE.** *Added 2026-07-28 from Jake's report: "Signs out, but can't sign in as anyone else in Safari."* Not Safari and not `signOut` — `signInWithPopup` with a bare provider asks Google for "the signed-in user", and where that is unambiguous Google answers **without drawing a chooser**. Fixed in store 0.21.2 with `prompt: "select_account"`. After ⏻, signing in must **offer a list of accounts**, including *Use another account*. **Everything two-person below depends on this one.**

Group **KEYS** — boards, switching, visiting.

1. ⚪ **KEYS-1. One key, no switcher.** *Rewritten — "I don't know what this means" was a fair complaint.* If your account holds a key to exactly **one** board, there should be **no board-switcher chip in the header at all**, because a menu with one item is furniture. **Jake can no longer test this**: he holds his own board plus Nico's. It was true on his first sign-in and will be true for the next brand-new account. Retire it if nobody creates one.
2. ⏳ **KEYS-2. Somebody invites you to a whole BOARD.** They sign in, ⚙️ ▸ **People**, your address, "Can edit", *Give them a key*. **Nothing for you to accept.** Reload: the chip appears. *(Not yet run. A colleague has invited Jake to a **tier**, which is TIER-1 and passed — a different mechanism.)*
3. ✅ **KEYS-3. You visit their house.** Chip → their board: their tiers, their tasks, their projects. The chip goes dashed and reads *visiting* with their name. *Jake: he can visit Nico; Nico's queue is empty and Nico cannot see Jake's tasks — **that is the pass**, it is E1 holding. The "same documents, not a copy" half is proven by TIER-3 instead.*
4. ⏳ **KEYS-4. Your settings stay yours.** In their house you keep *your* view, *your* hidden tiers, *your* week layout — per-device, not per-board, deliberately.
5. ⏳ **KEYS-5. It remembers.** **One reload is the whole test.** Switch to Nico's board, **reload**, land back on Nico's. Switch to your own, reload, your own. *(Jake reported swapping back and forth works — that is KEYS-3. This one needs the reload.)*
6. ⏳ **KEYS-6. They can take the key back.** Removed in ⚙️ ▸ People → that board leaves your switcher on reload, and opening it directly fails.
7. ✅ **KEYS-7. Nico's board exists and is dependent.** ⚙️ ▸ People ▸ *A board for someone who doesn't own it* → name, his address, Katie as co-owner. Appears in your switcher and hers. He can use it fully, cannot remove either of you, cannot delete it, cannot leave it: **dependent** badge, no ✕.
8. ✅ **KEYS-8. THE ONE THAT MATTERS MOST — he signs in and lands in the right house.** Nico signs in for the first time *after* his board exists, and lands on **that** board, not a fresh one. Three starter tiers and no sign of you means the adoption path in `resolveWorkspace` did not fire — check for the missing collection-group index first.

Group **TIER** — shared tiers (item 5).

1. ✅ **TIER-1. A tier becomes a shared tier.** ⚙️ ▸ Tiers ▸ 🤝 ▸ their address ▸ *Share this tier*. It reports how many documents moved; **your queue looks identical afterwards**. Console: a workspace with `kind: "shared"` holding that tier and its tasks. *Passed from the RECEIVING side — a colleague shared one with Jake. The sending side is still unrun.*
2. ✅ **TIER-2. The switcher did NOT gain an entry.** *Jake: "I cannot see the colleague's board."* Exactly right — a shared tier is not a board you visit (§6.2).
3. ✅ **TIER-3. THE ONE THAT MATTERS MOST — same document, live, two real people.** *Jake, on his colleague's shared tier: "My colleague made an event and I can see the event! They're the same! I could check it off right now! In fact, I did, and I unchecked it at the same time!"* **This is item 5 working**, and it also retires KEYS-3's second half.
4. ⏳ **TIER-4. It went in whole.** Projects on that tier came across, and so did their clocked sessions — the Time Report still totals them. Most likely to be half-done, because sessions are matched to projects client-side.
5. ⏳ **TIER-5. New work lands on the right board.** Add a task to the shared tier; console says it is in the **shared** workspace. If it lands in yours, `wsOfTier` is not resolving and the feature is cosmetic.
6. ⏳ **TIER-6. Order is per person, and is a POSITION.** Reorder with ▲▼ — **there is no number to type any more** (app 1.30.0; see below). Your order and theirs both hold. `users/{you}.tierRanks` gains a `"wsId:tierId"` entry and your row in `workspaces/{shared}/members/` gains a `tierRanks` map.
7. ⏳ **TIER-7. Visiting is an honest picture.** Rank the shared tier differently from them and **save yours LAST**, then visit their board: it must show at **their** number. *Saving last is the test — run it the other way and both the broken and fixed versions pass.*
8. ⏳ **TIER-8. Jake's merge rule.** On someone's board, a tier you share **with them** is still there; a tier you share with a **third** person must **not** appear. Nothing else tests the rule he corrected.
9. ⏳ **TIER-9. Bring it back.** 🤝 ▸ *Bring it back to my board* → everything returns, the shared workspace **disappears from the console**, and they can no longer see any of it.
10. ⏳ **TIER-10. Undo lands on the right board.** Delete a task on a shared tier, undo, and confirm **in the console** that it returned to the **shared** workspace. Only test of the tombstone map, and **its failure is silent** — the task looks right on your screen and is gone forever from theirs.

**Item 7 — calendars.** Group **CAL-1…8**, still living in SETUP-PHASE3-2.0.md Part 6 as WW1–WW8. **CAL-8 is new and 1.x could not have had it:** the report must name more than one workspace, and one person's broken calendar share must show as `error` on *their* row while everybody else's row is fine.

### 5b-i. Closed by the 2026-07-28 drop

- **⚠️ EDITING A PROJECT'S STAGES WAS ERASING `completedBy` (E9).** Fixed in app 1.30.1. Jake found it in console data and read it as *"if Tomlinson checked it off she gets credit, if I do it does not"* — reasonable, and wrong in a way worth keeping. `setStageDone` writes `completedBy = whoami()` **unconditionally**, so the stamp cannot depend on who; and the field was **absent rather than null**, which means something removed it later. `saveStages()` rebuilt each stage from the form carrying an ALLOWLIST of two fields (`completedAt`, `dueAt`) and dropped everything else — so every stage edit left the stage ticked with nobody's name on it. **The real split in his data was TIME, not person:** his completions predated the last stage edit and hers followed it. Had she gone first, the evidence would have accused her.
  **Two things to keep from this.** (1) *A keep-list is a list somebody must remember to extend.* Now the whole stage is carried and only editor-owned fields are overridden — with `hurrah` explicitly stripped first, because D109 says the editor owns that flag and a stale `true` surviving a spread would resurrect a climax the user just switched off. (2) **The stamps already lost are NOT recoverable** — the value is gone from the document and nothing knows what it was.
  **This bug predates item 5 and was harmless until it shipped.** While one person authored everything, "who completed this" had one possible answer; a shared tier is what turned a silent field-drop into a false accusation.

- **The rank collision.** *Jake: "it came in as 4/4. I changed it to 3 without changing my 3, and it just… stayed there?"* A typed rank worked for four years because **one person authored every number**; item 5 ended that, and a guest tier now arrives carrying a stranger's number. Fixed in app 1.30.0 — ▲▼ arrows, rank assigned **1..N from DOM position on save**. His "it should kick back an error" is answered by making the state unconstructible instead: opening ⚙️ ▸ Tiers and saving also **normalises any collision already in the data**.
- **Sign-in as a second account.** store 0.21.2, above (BASE-9).
