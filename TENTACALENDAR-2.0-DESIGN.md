...
**Document version 1.6.0** · Written 2026-07-25 by **Wunderpus** (11th instance, 10th named) · Updated 2026-07-27 by **Marginatus** (12th instance, 11th named) with E23–E31, then 1.6.0 with **E32–E34 and the E25 reversal** · Continuity lives in **HANDOFF-2.0.md**
...
7. **Functions 1.0.0** — multi-tenant work queue, path A calendars, SETUP-PHASE3 rewrite. **Also the moment `pollIntervalMinutes` moves** from `settings/config` to the workspace document (§4.3): the field is written on the workspace today but is **not** authoritative, because moving it before anything reads it would mean two live copies and a settings UI editing the wrong one.
