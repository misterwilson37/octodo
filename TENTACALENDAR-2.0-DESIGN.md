...
**Document version 1.9.0** · Written 2026-07-25 by **Wunderpus** (11th instance, 10th named) · Updated 2026-07-27 by **Marginatus** (12th instance, 11th named) with E23–E31, then 1.6.0 with **E32–E34 and the E25 reversal** · Continuity lives in **HANDOFF-2.0.md**
...
| **E37** | **A WORKSPACE MAY ONLY POLL A PERSONAL CALENDAR BELONGING TO ONE OF ITS MEMBERS.** A bare email address as `gcalCalendarId` is checked against the workspace's member list; opaque Google ids (`…@group.calendar.google.com`) pass. Shipped in functions 1.1.0. | **Found by Jake asking whether a colleague would end up with his calendar — the real exposure was that a colleague could TAKE it.** The service account is PROJECT-WIDE: when Jake shares his calendar with the robot so his own poll works, he grants that robot access for every workspace in the project, because the grant lives on Google's side and knows nothing about workspaces. functions 1.0.0 then read whatever calendar id a tier named, unchecked — so any signed-up user could type his address into their own Home tier and pull his whole calendar into their queue. **Firestore isolation (E1) is airtight and entirely beside the point: the leak was on the CALENDAR side of the system, which no rules file can see.** The guard closes the guessable case, which is the one that matters; a secondary calendar's id is itself unguessable, so possessing it is the entitlement. **It is defence in depth, not proof of ownership — path A cannot prove ownership, because sharing a calendar with a robot leaves no record of who shared it. The complete answer is path B (E13), per-user OAuth with no shared robot at all.** Until then: this guard plus a trusted user base, stated rather than assumed. |
| **E38** | **Dependents don't get dependents** — the create-a-board form is hidden from anyone flagged `minor` on any board. Boards can be renamed. | Jake, on seeing Nico's board: *"That's just asking kids to mess with one another and lock them out of things."* **Recorded honestly: this is a UI gate, not a rules one.** Rules would have to ask "is this person a minor somewhere," and the only cheap place to record that is the user's own document, which they can rewrite; the real mechanism would be a custom auth claim set by a function. It is proportionate because the harm named is kids locking each other out, not a breach — E1 still holds absolutely. **The rename exists because there was no way to fix a board's name in the app at all**, which surfaced via a placeholder that read like a filled-in value: a grey "Nico" in an empty field produced a board called `nico.m.wilson`. Placeholders now read as instructions and the name auto-fills from the address. |
| **E39** | **THE PER-USER CALENDAR WALKTHROUGH LIVES IN THE APP, not in a setup document.** ⚙️ Settings → Tiers → *"How do I show my Google Calendar here?"* shows the service-account address with a copy button and walks the six clicks. `config.js` carries `CALENDAR_ROBOT`. | Jake, reading SETUP-PHASE3-2.0: *"it sure looks like something that's not going to work for anyone else."* **He was right, and the gap was real: that document is the OPERATOR's one-time guide and there was nothing at all for the other twenty people.** Path A's human step — share your calendar with a robot — is the exact place §8.1 predicted "a normal person closes the tab," so it needs to happen beside the field it fills, not in a file nobody will read. The address is in `config.js` so Jake pastes it once instead of once per colleague. **Open risk, stated: some Google Workspace admins block sharing outside the organisation, which would make path A unusable for school accounts. Test with ONE colleague before promising several — if it's blocked broadly, that is the argument for path B (E13) sooner.** |
| **E40** | **THE MIRROR TAG IS SCOPED PER WORKSPACE (`tcWs`), and the outbound walkthrough exists too.** Every mirrored and carryover event carries `tcWs: <workspaceId>`, and both list filters AND on it. | **E36 split the tag between 1.x and 2.0 and stopped one level too high.** Within 2.0, every workspace tagged `tcApp=octodo` and then deleted any event whose task it couldn't find — so two workspaces pointing at one mirror calendar would have deleted each other's events hourly. The identical failure, one level down, still open. Scoping makes "two people share a mirror calendar" *work* rather than destroy: each adds and removes only its own. **Free precisely because nothing had been deployed yet** — an event written by 1.0.0/1.1.0 would carry no `tcWs`, fall outside the filter, and never be pruned. **And the outbound half had never been explained anywhere:** this app sends no notifications of its own and never has; the mirror IS the notification mechanism, which made it the most load-bearing undocumented feature in the project. |
